...
public sealed class AutoAppLaunchService : IDisposable
{
    ...
    private void UpdateChildProcesses()
    {
        foreach (var pair in startedProcesses.ToArray())
        {
            ...
            {
                bool recursiveChildSearch =
                    processes.Count ==
                    1; // Disable recursion when this list may already contain the entire process tree
    ...
    private bool IsProcessRunning(string filePath)
    {
        ...
        {
            _logger.Error(ex, "Error checking if process is running: {0}", filePath);
            return false;
        }
    }
