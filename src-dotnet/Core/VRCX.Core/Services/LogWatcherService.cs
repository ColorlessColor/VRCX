...
public sealed class LogWatcherService : IDisposable
{
    ...
    private readonly ReaderWriterLockSlim _logListLock = new();

    private bool _firstRun = true;
    private bool _resetLog;
    ...
    private readonly ConcurrentQueue<string> _logQueue = new(); // for electron

    // NOTE
    // FileSystemWatcher() is unreliable

    public LogWatcherService(IGameFolderProvider gameFolderProvider, IMainWebViewService mainWebViewService)
    {
        _mainWebViewService = mainWebViewService;
        var logPath = gameFolderProvider.GetVRChatAppDataLocation();
    ...
    private void Update()
    {
        if (_resetLog)
        {
            _firstRun = true;
            _resetLog = false;
            _logContextMap.Clear();
            _logListLock.EnterWriteLock();
            try
            {
                _logList.Clear();
            }
            ...
            {
                _logListLock.ExitWriteLock();
            }
        }

        var deletedNameSet = new HashSet<string>(_logContextMap.Keys);
        _logDirectoryInfo.Refresh();

        if ((_logDirectoryInfo.LinkTarget == null && _logDirectoryInfo.Exists) ||
            Directory.Exists(_logDirectoryInfo.LinkTarget))
        {
            var fileInfos = _logDirectoryInfo.GetFiles("output_log_*.txt", SearchOption.TopDirectoryOnly);
            ...
            foreach (var fileInfo in fileInfos)
            {
                ...
                if (DateTime.Compare(fileInfo.LastWriteTimeUtc, _tillDate) < 0)
                    continue;

                if (_logContextMap.TryGetValue(fileInfo.Name, out var logContext))
                {
                ...
                else
                {
                    ...
                    _logContextMap.Add(fileInfo.Name, logContext);
                }
        ...
        try
        {
            ...
            while (true)
            {
                    ...
                    ))
                {
                    ...
                    if (DateTime.Compare(lineDate, _tillDate) <= 0)
                    {
    ...
    private bool ParseLogAvatarPedestalChange(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        if (string.Compare(line, offset, "[Network Processing] RPC invoked SwitchAvatar on AvatarPedestal for ", 0, 68,
    ...
    private bool ParseLogVideoChange(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        if (string.Compare(line, offset, "[Video Playback] Attempting to resolve URL '", 0, 44,
                StringComparison.Ordinal) != 0)
            return false;
    ...
    private bool ParseLogUsharpVideoPlay(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        if (string.Compare(line, offset, "[USharpVideo] Started video load for URL: ", 0, 42,
                StringComparison.Ordinal) != 0)
            return false;
    ...
    private bool ParseApplicationQuit(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        if (string.Compare(line, offset, "VRCApplication: OnApplicationQuit at ", 0, 37, StringComparison.Ordinal) !=
            0 &&
            string.Compare(line, offset, "VRCApplication: HandleApplicationQuit at ", 0, 41,
                StringComparison.Ordinal) != 0)
            return false;
    ...
    private bool ParseVoteKick(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        if (string.Compare(line, offset, "[Behaviour] Received executive message: ", 0, 40, StringComparison.Ordinal) !=
            0)
            return false;
    ...
    private bool ParseVoteKickInitiation(FileInfo fileInfo, LogContext logContext, string line, int offset)
    {
        ...
        int index = line.IndexOf("[ModerationManager] A vote kick has been initiated against ",
            ...
            _logList.Count > 0)
        {
            ...
            try
            {
                ...
                if (_logList.Count > 1000)
                {
                    ...
                    _logList.CopyTo(0, items, 0, 1000);
                    _logList.RemoveRange(0, 1000);
                }
                else
                {
                    items = _logList.ToArray();
                    _logList.Clear();
                }
