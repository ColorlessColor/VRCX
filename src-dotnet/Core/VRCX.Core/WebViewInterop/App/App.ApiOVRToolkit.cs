...
namespace VRCX.Core.WebViewInterop.App
{
    ...
    {
        private static readonly byte[] VrcxIcon = Convert.FromBase64String(
        ...
        public async Task OVRTNotification(bool hudNotification, bool wristNotification, string title, string body,
            int timeout, double opacity, string image = "")
        {
            List<Task> sendTasks = [];
            ...
            try
            {
                ...
                {
                    imageBytes = await File.ReadAllBytesAsync(image);
                }
            }
            catch (Exception ex)
            {
            ...
            {
                sendTasks.Add(
                    ovrToolkitClientService.SendWristNotificationAsync(new OvrToolkitWebSocketWristNotificationMessage
                    {
                        Body = title + " - " + body
                    })
                );
            ...
            {
                sendTasks.Add(
                    ovrToolkitClientService.SendHudNotificationAsync(new OvrToolkitWebSocketHudNotificationMessage
                    {
                        Title = title,
                        Body = body,
                        Icon = imageBytes
